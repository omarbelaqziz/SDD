...
void partie_fractionnaire(int* entier, float* reel,int Neg){
    float dix=0.1;
    char car;
...
void partie_naturel(int* entier, float* reel, int *estReel){
    char car;
    int Neg = 1;
    if((car=getchar())=='-') Neg = -1;
    else goto label;
    ...
    while(car = getchar()){
        ...
        case '\n':
        case '\t':
        case ' ': *entier *= Neg; return;
        case '.': *estReel = 1;partie_fractionnaire(entier,reel,Neg);return;
...
void main(){
    ...
    partie_naturel(&valeur,&reel,&estReel);
    if(estReel) printf("la valeur que vous avez entré est %f", reel);
    else printf("la valeur que vous avez entré est %d", valeur);
}